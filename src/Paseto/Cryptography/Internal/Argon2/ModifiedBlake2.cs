...
{
    private static ulong Rotate(ulong x, int y) => (((x) >> (y)) ^ ((x) << (64 - (y))));

    private static unsafe void ModifiedG(ulong* v, int a, int b, int c, int d)
    {
    ...
    public static unsafe void DoRoundRows(ulong* v, int i)
    {
        i *= 2;
    ...
    public static void Blake2Prime(Memory<ulong> memory, LittleEndianActiveStream dataStream, int size = -1)
    {
        ...
        if (size <= 64)
        {
            var blake2 = new Blake2bMac(8 * size);
            blake2.Initialize();
            memory.Span.Blit(blake2.ComputeHash(hashStream).AsSpan()[..size], 0);
        }
        else
        {
            var blake2 = new Blake2bMac(512);
            blake2.Initialize();

            var offset = 0;
            var chunk = blake2.ComputeHash(hashStream);

            memory.Span.Blit(chunk.AsSpan()[..32], offset); // copy half of the chunk
            offset += 4;
            size -= 32;
            ...
            while (size > 64)
            {
                ...
                memory.Span.Blit(chunk.AsSpan()[..32], offset); // half again

                offset += 4;
                size -= 32;
            }

            blake2 = new Blake2bMac(size * 8);
            blake2.Initialize();
            memory.Span.Blit(blake2.ComputeHash(chunk).AsSpan()[..size], offset); // copy the rest
        }
    }
}